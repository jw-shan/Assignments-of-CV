...
\author[Jiawei Shan]{\small{Github Page: \url{http://github.com/beurtschipper/Depix}}\\
~\\
...
\begin{enumerate}
	\item What is Pixelization?
	\item How to implement pixelization?
	\item How to recover from pixelization?
\end{enumerate}
...
\begin{itemize}
	\item Since the linear box filter is a deterministic algorithm, pixelizing the same values will always result in the same pixelated block.
	...
	\item The key to find matching is that the exact block of the same configuration of pixels to exist in the search image.
...
\begin{frame}{Conclusion}
	...
	{\bl Limitations:}
	\begin{itemize}
		\item The expected characters which is possible to exist in the image should be given in advance with the same font settings (text size, font, color, hsl) and in the same editor.
		\item This algorithm is helpless to recover general images from pixelization.
		\item An image with both spaced and close letters takes longer to search.
	\end{itemize}
\end{frame}
